...
export default async function get(c: Context) {
    ...
    let query = spClSess
        ...
        .select(`
            created_at,
            user_rel_id,
            users_master!users_lines_follows_user_rel_id_fkey(
                in_spbs_id,
                handle_id
            )
        `)
        .eq('target_user_rel_id', targetUserRelId)
    ...
    if (restartUuidParam) {
        ...
        if (!restartUserError && restartUser?.rel_id) {
            query = query.lt('user_rel_id', restartUser.rel_id);
        }
    }
    ...
    const { data: followersData, error: followersError } = await query;
    ...
    for (const follower of followersData) {
        if (!follower.users_master || !Array.isArray(follower.users_master) || follower.users_master.length === 0) continue;

        const followerUser = follower.users_master[0];
        const followerUuid = followerUser.in_spbs_id;
        const handleId = followerUser.handle_id;

        const { data: profileData } = await spClSess
            .from(viewName)
            .select('display_name, description, icon, icon_path')
            .eq('rel_id', follower.user_rel_id)
            .single();
