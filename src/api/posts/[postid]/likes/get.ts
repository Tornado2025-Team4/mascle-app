...
export default async function get(c: Context) {
  ...
  let query = (spClSess || spClAnon)
    .from('views_user_post_likes')
    .select('*')
    .eq('post_pub_id', postid)
    .order('liked_at', { ascending: false });

  if (rq.before_liked_at) {
    query = query.lt('liked_at', rq.before_liked_at);
  }
  ...
  if (error) {
    throw new ApiErrorFatal(`failed to fetch likes: ${error.message}`);
  }
