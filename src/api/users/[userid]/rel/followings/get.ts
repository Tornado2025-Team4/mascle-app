...
export default async function get(c: Context) {
    ...
    const { data: followingsData, error: followingsError } = await spClSA
        .from('views_user_rel')
        .select('followed_user_pub_id')
        .eq('follower_user_pub_id', userIdInfo.pubId)
        .eq('privacy_allowed', true)
        .limit(limit);

    if (followingsError) {
        throw new ApiErrorFatal(`Failed to get followings: ${followingsError.message}`);
    }

    const followingPubIds = followingsData.map(f => f.followed_user_pub_id);

    if (followingPubIds.length === 0) {
        return c.json([]);
    }
