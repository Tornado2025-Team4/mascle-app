  ...
  return (
    <footer className="fixed bottom-0 left-0 w-full h-[10vh] bg-white border-t border-gray-200 flex items-center justify-center z-50 safe-area-inset-bottom">
      <nav role="navigation" aria-label="メインナビゲーション" className="w-full max-w-md">
        <ul className="flex items-center justify-around px-4 text-2xl">
          {/* Home * /}
<li>
  <Link
    href="/"
    aria-label="ホーム"
    className={`p-3 rounded-lg transition-colors duration-200 ${isActive('/') ? 'text-blue-600' : 'text-gray-600 hover:text-gray-800'
      }`}
  >
    {isActive('/') ? (
...
<li>
  <Link
    href="/explore"
    aria-label="検索・探索"
    className={`p-3 rounded-lg transition-colors duration-200 ${isActive('/explore') ? 'text-blue-600' : 'text-gray-600 hover:text-gray-800'
      }`}
  >
    {isActive('/explore') ? (
...
<li>
  <Link
    href="/create_post"
    aria-label="新規投稿"
    className={`p-3 rounded-lg transition-colors duration-200 ${isActive('/create_post') ? 'text-blue-600' : 'text-gray-600 hover:text-gray-800'
      }`}
  >
...
<li>
  <Link
    ...
    className={`p-3 rounded-lg transition-colors duration-200 ${isActive(profileHref) ? 'text-blue-600' : 'text-gray-600 hover:text-gray-800'
